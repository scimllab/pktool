...
process_domain <- function(file_name,
  ...
  if(grepl(".xpt",file_name,ignore.case = TRUE)){df<-as.data.frame(lapply(file.path(projpath, file_name),read_xpt))}
  else if(grepl(".sas7dat",file_name,ignore.case = TRUE)){df<--as.data.frame(lapply(file.path(projpath, file_name),read_sas))}
  else if(grepl(".csv",file_name,ignore.case = TRUE)){df<--as.data.frame(lapply(file.path(projpath, file_name),read.csv))}
